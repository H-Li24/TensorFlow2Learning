...
class my_test_model(tf.keras.Model): # 传入tf.keras.Model
...
My_Test_Model = my_test_model(class_number=10) # 传入的变量要指定参数名

My_Test_Model.compile(optimizer=tf.keras.optimizers.Adam(0.01), loss=tf.keras.losses.CategoricalCrossentropy(from_logits=True), metrics=["accuracy"])
